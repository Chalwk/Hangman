...
local function updateOptionsButtonPositions(self)
    ...
    for _, button in ipairs(self.optionsButtons) do
        ...
        elseif button.section == "category" then
            ...
            button.y = cateY + math_floor(cateIndex / 2) * (cateButtonH + 15)
...
local function drawOptionsInterface(self)
    ...
    lg.printf("Category", 0, startY + 90, self.screenWidth, "center")

    updateOptionsButtonPositions(self)
    drawOptionSection(self, "difficulty")
    drawOptionSection(self, "category")
...
function Menu:setCategory(category) self.category = category end

function Menu:getCategory() return self.category end
